...
#pragma mark - Begin Lamy SDK integration
- (void)configureDrawViewForAdonitSDK
{
//    [[LamyStylusManager sharedInstance] registerView:self];
}

#pragma mark - LamyStrokeDelegate

/**
 * Handles the start of a stylus stroke
 */
- (void)lamyStylusStrokeBegan:(LamyStroke *)stylusStroke
{
    self.viewController.protoController.stylusIsOnScreen = YES;
    [self updateRadiusViewsForLamyStylus:stylusStroke andPhase:UITouchPhaseBegan];
    [self checkForShadingWithTilt:stylusStroke.altitudeAngle];
    [self.viewController cancelTap];
    NSInteger coalesedCounter = 0;
    LamyStroke *lastCoalescedStroke = [stylusStroke.coalescedLamyStrokes lastObject];
    ...
    for (LamyStroke *coalescedLamyStroke in stylusStroke.coalescedLamyStrokes) {
        ...
        coalesedCounter++;
        CGFloat pressure = coalescedLamyStroke.pressure;
        CGPoint location = [coalescedLamyStroke locationInView:self];
        self.viewController.radianLabel.text = [NSString stringWithFormat:@"%.5f",[coalescedLamyStroke altitudeAngle]];
        self.viewController.angleLabel.text = [NSString stringWithFormat:@"%.5f",[coalescedLamyStroke altitudeAngle] * 180 / M_PI];
        CGFloat offset = 0;//[self widthForPressure:pressure tilt:coalescedLamyStroke.altitudeAngle] / 10;
        
        [self addLineToAndRenderStroke:currentStroke
                               toPoint:CGPointMake(location.x + offset, location.y + offset)
                               toWidth:[self widthForPressure:pressure tilt:coalescedLamyStroke.altitudeAngle]
                               toColor:[self colorForPressure:pressure tilt:coalescedLamyStroke.altitudeAngle]
                              withPath:nil
                          shouldRender:coalescedLamyStroke.timestamp == lastCoalescedStroke.timestamp
                      coalescedInteger:coalesedCounter];
    }
    [self.viewController.lamyStatusIndicatorContainerView.pressureLabel setText:[NSString stringWithFormat:@"%.10f", stylusStroke.pressure]];
    [self.viewController.lamyStatusIndicatorContainerView.rawPressureLabel setText:[NSString stringWithFormat:@"%lu", (unsigned long)stylusStroke.rawPressure]];
}

/**
 * Handles the continuation of a stylus stroke.
 */
- (void)lamyStylusStrokeMoved:(LamyStroke *)stylusStroke
{
    [self updateRadiusViewsForLamyStylus:stylusStroke andPhase:UITouchPhaseMoved];
    [self checkForShadingWithTilt:stylusStroke.altitudeAngle];
    ...
    LamyStroke *lastPredictedStroke = [stylusStroke.predictedLamyStrokes lastObject];
    LamyStroke *lastCoalescedStroke = [stylusStroke.coalescedLamyStrokes lastObject];
    SmoothStroke *currentStroke = [self getStrokeForHash:@(stylusStroke.hash)];
    ...
    for (LamyStroke *coalescedLamyStroke in stylusStroke.coalescedLamyStrokes) {
        ...
        CGPoint location = [coalescedLamyStroke locationInView:self];
//        NSLog(@"azimuthAngleInView = %f , azimuthUnitVectorInView = %@ , altitudeAngle = %f",[coalescedLamyStroke azimuthAngleInView:self] ,NSStringFromCGVector([coalescedLamyStroke azimuthUnitVectorInView:self]), [coalescedLamyStroke altitudeAngle] );
        self.viewController.radianLabel.text = [NSString stringWithFormat:@"%.5f",[coalescedLamyStroke altitudeAngle]];
        self.viewController.angleLabel.text = [NSString stringWithFormat:@"%.5f",[coalescedLamyStroke altitudeAngle] * 180 / M_PI];
        
        CGFloat pressure = coalescedLamyStroke.pressure;
        CGFloat width = [self widthForPressure:pressure tilt:coalescedLamyStroke.altitudeAngle];
        CGFloat offset = 0;//[self widthForPressure:pressure tilt:coalescedLamyStroke.altitudeAngle] / 10;
        ...
        [self addLineToAndRenderStroke:currentStroke
                               ...
                               toColor:[self colorForPressure:pressure tilt:coalescedLamyStroke.altitudeAngle]
                              withPath:nil
                          shouldRender:coalescedLamyStroke.timestamp == lastCoalescedStroke.timestamp && !stylusStroke.predictedLamyStrokes.count
                      coalescedInteger:stylusStroke.coalescedLamyStrokes.count];
    }
    ...
    {
        min = max = [((LamyStroke*)stylusStroke.coalescedLamyStrokes.firstObject) locationInView:self];
        for (LamyStroke *coalescedStroke in stylusStroke.coalescedLamyStrokes) {
        ...
        for (LamyStroke *predictedStroke in stylusStroke.predictedLamyStrokes) {
    ...
    if(stylusStroke.predictedLamyStrokes.count){
        ...
        for (LamyStroke* predictedStroke in stylusStroke.predictedLamyStrokes){
            ...
            [self addLineToAndRenderStroke:currentStroke
                          ...
                          coalescedInteger:stylusStroke.predictedLamyStrokes.count];
            ...
            if (predictedStroke.timestamp == lastPredictedStroke.timestamp) {
                ...
                [self addLineToAndRenderStroke:currentStroke
                              ...
                              coalescedInteger:stylusStroke.predictedLamyStrokes.count + 1];
                //NSLog(@"prediction Location: %@", NSStringFromCGPoint([predictedStroke locationInView:nil]));
            }
        }
    }
    //Set LamyTouchStatusIndicator labels
    [self.viewController.lamyStatusIndicatorContainerView.pressureLabel setText:[NSString stringWithFormat:@"%.10f", stylusStroke.pressure]];
    [self.viewController.lamyStatusIndicatorContainerView.rawPressureLabel setText:[NSString stringWithFormat:@"%lu", (unsigned long)stylusStroke.rawPressure]];
}
 ...
 */
- (void)lamyStylusStrokeEnded:(LamyStroke *)stylusStroke
{
    [self updateRadiusViewsForLamyStylus:stylusStroke andPhase:UITouchPhaseEnded];
    ...
    for (LamyStroke *coalescedLamyStroke in stylusStroke.coalescedLamyStrokes) {
        ...
        self.viewController.radianLabel.text = [NSString stringWithFormat:@"%.5f",[coalescedLamyStroke altitudeAngle]];
        self.viewController.angleLabel.text = [NSString stringWithFormat:@"%.5f",[coalescedLamyStroke altitudeAngle] * 180 / M_PI];
        CGFloat stylusPressure = coalescedLamyStroke.pressure / 2.0; // Setting end of each stroke to zero pressure can cause a more organic stroke roll off with fast strokes.
        [self addLineToAndRenderStroke:currentStroke
                               toPoint:location
                               toWidth:3//[self widthForPressure:stylusPressure tilt:coalescedLamyStroke.altitudeAngle]
                               toColor:[self colorForPressure:stylusPressure tilt:coalescedLamyStroke.altitudeAngle]
                              withPath:nil
                          shouldRender:NO //coalescedLamyStroke.timestamp == lastCoalescedStroke.timestamp
                      coalescedInteger:stylusStroke.coalescedLamyStrokes.count];
        
        if (coalescedLamyStroke.timestamp == lastCoalescedStroke.timestamp) {
            ...
            [self addLineToAndRenderStroke:currentStroke
                                   toPoint:location
                                   toWidth:3//[self widthForPressure:endingPressure tilt:coalescedLamyStroke.altitudeAngle]
                                   toColor:[self colorForPressure:endingPressure tilt:coalescedLamyStroke.altitudeAngle]
                                  withPath:nil
                              shouldRender:coalescedLamyStroke.timestamp == lastCoalescedStroke.timestamp
                          coalescedInteger:stylusStroke.coalescedLamyStrokes.count + 1];
        }
    }
    
    [self cleanupEndedStroke:currentStroke forHash:@(stylusStroke.hash)];
    
    //Set LamyTouchStatusIndicator labels back to default
    [self.viewController.lamyStatusIndicatorContainerView.pressureLabel setText:@"none"];
    [self.viewController.lamyStatusIndicatorContainerView.rawPressureLabel setText:@"none"];
}

/**
 * Handles the cancellation of a stylus stroke event.
 */
- (void)lamyStylusStrokeCancelled:(LamyStroke *)stylusStroke
{
...
{
    NSLog(@"suggestsToDisableGestures");
    // disable any other gestures, like a pinch to zoom
    [self disableGestures];
    [self.viewController handleSuggestsToDisableGestures];
}

- (void)suggestsToEnableGestures
{
    NSLog(@"suggestsToEnableGestures");
    // enable any other gestures, like a pinch to zoom
    [self enableGestures];
    [self.viewController handleSuggestsToEnableGestures];
}

#pragma mark - UITouch Events

/**
 * If the LamySDK is enabled, then all Lamy stylus
 * events will be sent to the lamyStylus: delegate methods.
 * All touches, regardless of if they map to a Lamy stylus
        ...
        for (UITouch *mainTouch in touches) {
            ...
            for (UITouch *coalescedTouch in coalescedTouches) {
                ...
                [self addLineToAndRenderStroke:[self getStrokeForHash:@(currentStroke.hash)]
                                       toPoint:location
                                       toWidth:[self widthForPressure:(CGFloat)[LamyStylusManager sharedInstance].unconnectedPressure / (CGFloat)LAMY_STYLUS_MAX_PRESSURE tilt:M_PI_2]
                                       toColor:[self colorForPressure:(CGFloat)[LamyStylusManager sharedInstance].unconnectedPressure / (CGFloat)LAMY_STYLUS_MAX_PRESSURE tilt:M_PI_2]
                                      withPath:nil
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (![LamyStylusManager sharedInstance].isStylusConnected) {
        ...
        for (UITouch *mainTouch in touches) {
            ...
            SmoothStroke* currentStroke = [self getStrokeForHash:@(mainTouch.hash)];
            CGFloat brushWidth = [self widthForPressure:(CGFloat)[LamyStylusManager sharedInstance].unconnectedPressure / (CGFloat)LAMY_STYLUS_MAX_PRESSURE tilt:M_PI_2];
...
//                NSLog(@"azimuthAngleInView = %f , azimuthUnitVectorInView = %@ , altitudeAngle = %f",[coalescedTouch azimuthAngleInView:self] ,NSStringFromCGVector([coalescedTouch azimuthUnitVectorInView:self]), [coalescedTouch altitudeAngle] );
            ...
            for (UITouch* predictedTouch in predictedTouches){
                UIColor *predictionColor = [self colorForPressure:(CGFloat)[LamyStylusManager sharedInstance].unconnectedPressure / (CGFloat)LAMY_STYLUS_MAX_PRESSURE tilt:M_PI_2];
...
- (CGFloat)percentageOfTiltEffectToAppyFromCurrentTilt:(CGFloat)tilt
{
    ...
    if ([LamyStylusManager sharedInstance].minimumAltitudeAngleSupported >= 0.5) {return 0.0;}
...
- (void)dealloc
{
//    [[LamyStylusManager sharedInstance] unregisterView:self];
...
{
    if (touch.lamyTouchIdentification == LamyTouchIdentificationTypeStylus) {
...
- (void)updateRadiusViewsForTouches:(NSSet *)touches
{
    if (self.showRadiusViews) {
        for (UITouch *touch in touches) {
            LamyRadiusView *radiusView = [self getRadiusViewForHash:@(touch.hash)];
...
- (LamyRadiusView *)getRadiusViewForHash:(NSNumber *)hash
{
    LamyRadiusView *radiusView = [self.currentRadiusViews objectForKey:hash];
    if (!radiusView) {
        radiusView = [[LamyRadiusView alloc]initWithFrame:CGRectZero];
...
- (void)cleanupRadiusViewForHash:(NSNumber *)hash
{
    ...
    __block LamyRadiusView *radiusView = [self getRadiusViewForHash:hash];
...
- (void)cleanupAllRadiusView {
    for (LamyRadiusView *radiusview in [self.currentRadiusViews objectEnumerator]) {
...
- (void)updateRadiusViewsForLamyStylus:(LamyStroke *)stylusStroke andPhase:(UITouchPhase)phase
{
    if (self.showRadiusViews) {
        for (LamyStroke *coalescedLamyStroke in stylusStroke.coalescedLamyStrokes) {
            LamyRadiusView *radiusView = [self getRadiusViewForHash:@(coalescedLamyStroke.hash)];
            if (phase != UITouchPhaseCancelled && phase != UITouchPhaseEnded) {
                [radiusView updateViewWithLamyStroke:coalescedLamyStroke];
            } else {
                [radiusView updateViewWithLamyStroke:coalescedLamyStroke];
                //                [self cleanupRadiusViewForHash:@(coalescedLamyStroke.hash)];
            }
        }
    }
}
