...
+ (void)showLamyHUDInView:(UIView *)viewToDisplayIn isConnected:(BOOL) isConnected modelName:(NSString *) modelName
{
    ...
    for (LamyStatusHUD *HUDtoRemove in viewToDisplayIn.subviews)
    {
        // Make sure it is a JotTouchStatusHUD we remove
        if ([HUDtoRemove isKindOfClass:[LamyStatusHUD class]])
        {
            [[LamyStatusHUD class] removeJotStatusHUD:HUDtoRemove];
        }
    }

    LamyStatusHUD *HUDToDisplay = [self returnJotTouchStatusHUDWithConnection:isConnected modelName:modelName];
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        [[LamyStatusHUD class]removeJotStatusHUD:HUDToDisplay];
    });
...
+ (void)showLamyHUDInView:(UIView *)viewToDisplayIn topLineMessage:(NSString *)topLineMessage bottomeLineMessage:(NSString *)bottomLineMessage
{
    ...
    LamyStatusHUD *HUDToDisplay = [self returnJotTouchStatusHUDWithTopLineMessage:topLineMessage bottomeLineMessage:bottomLineMessage];
...
+ (void)AddCenterConstraintsForHUDView:(LamyStatusHUD *)HUD superView:(UIView *)superView
{
...
+ (void)removeJotStatusHUD:(LamyStatusHUD *)HUDtoRemove;
{
    [HUDtoRemove removeFromSuperview];
    HUDtoRemove = nil;
}

+ (LamyStatusHUD *)returnJotTouchStatusHUDWithTopLineMessage:(NSString *)topLineMessage bottomeLineMessage:(NSString *)bottomLineMessage
{
    CGRect HUDframe = CGRectMake(0, 0, HUD_WIDTH, HUD_HEIGHT);
    LamyStatusHUD *theHUD = [[LamyStatusHUD alloc]initWithFrame:HUDframe];
...
+ (LamyStatusHUD *)returnJotTouchStatusHUDWithConnection:(BOOL)isConnected modelName:(NSString *)modeName
{
    CGRect HUDframe = CGRectMake(0, 0, HUD_WIDTH, HUD_HEIGHT);
    LamyStatusHUD *theHUD = [[LamyStatusHUD alloc]initWithFrame:HUDframe];
